...
### 학습 방향: AI 에이전트를 배우는 학생으로서 무엇을 먼저 빌드업할 것인가

위 결론을 뒤집어 보면 학생 입장에서의 실천 지침이 나온다. 이번 프로젝트를 무너뜨린 원인
3가지(코퍼스 필터 버그, 표기 분열, 관계별 추출 recall 차이)는 전부 **모델 선택과 무관**했다
— gpt-4o-mini를 더 좋은 모델로 바꿔도 고쳐지지 않았을 문제들이었고, 실제로 고쳐진 것들
(허브 상한, 경로 연결성 검증, 정규화 사전)은 전부 **엔지니어링**이었다. 즉 이 도메인에서
실력을 가르는 것은 "모델을 잘 다루는 능력"이 아니라 "시스템이 왜 틀리는지 원인을 층위별로
좁혀서 고치는 능력"이라는 뜻이고, 여기서 세 가지 우선순위가 나온다.

**1순위 — 데이터·평가 엔지니어링** (이번 실패의 대부분이 여기서 났다)
- **골든셋 설계**: 정답만이 아니라 "기대 경로 + 원문 근거"까지 미리 정해두고 채점하는 습관.
  이게 없었다면 q12의 진짜 원인(특정 관계 타입의 낮은 추출 recall)을 절대 못 찾았을 것이다.
- **엔티티 정규화(entity resolution)**: 같은 개체가 다른 표기로 쪼개지는 문제는 모든
  지식그래프·RAG 프로젝트에서 반복된다. 별칭 사전 구축, 결과를 직접 눈으로 훑어 검증하는
  루틴은 프롬프트 트릭보다 오래 가는 자산이다.
- **recall/precision을 층위별로 나눠서 측정하는 습관**: "정확도 80%"처럼 뭉뚱그리지 않고
  관계 타입별·홉수별로 쪼개 보는 것. 이번 3장의 홉수별 표와 실패 분류가 그 예시다.

**2순위 — "LLM에게 무엇을 맡기고 무엇을 안 맡길지" 판단하는 아키텍처 감각**
- 정답이 결정적으로 존재하는 문제(그래프 경로 탐색 등)는 알고리즘에 맡기고, LLM은 애매한
  판단(자연어 이해, 후보 중 선택, 설명 생성)에만 쓰는 설계 감각. 이번에 "LLM이 서브그래프를
  통째로 보고 체인을 스스로 조립"하게 한 지점이 3홉에서 무너졌다 — 이 역할을 알고리즘으로
  넘겼다면 무너지지 않았을 것이다(위 "해법" 3번).
- 이는 LangGraph 같은 오케스트레이션 프레임워크의 State 설계와도 직결된다 — 어떤 정보를
  State에 명시적으로 담아 코드가 검증할 수 있게 만들지에 대한 감각.

**3순위 — 실패를 재현 가능하게 만드는 디버깅 습관**
- `AnswerResult`에 `reasoning` 필드를 추가해 모델을 블랙박스로 두지 않고 "생각의 흔적"을
  구조화 출력으로 남긴 것이 이번 프로젝트에서 가장 결정적인 순간이었다(q11의 원인을 코드
  버그가 아니라 모델의 다중 후보 탐색 한계로 정확히 좁힐 수 있었다).

반대로 **최신 모델 스펙이나 프롬프트 트릭을 따라잡는 것은 상대적으로 낮은 우선순위**다 —
틀린 방향은 아니지만 빨리 진부해지는 반면, "데이터를 의심하고 검증하는 습관"과 "실패를
층위별로 분류하는 방법론"은 모델이 몇 세대를 거쳐도 그대로 쓸 수 있는 능력이기 때문이다.
한 문장으로 요약하면: **"어떤 모델을 쓸까"를 고민하는 시간의 절반 이상을 "내 데이터와 내
평가셋이 정말 믿을 만한가"를 고민하는 데 쓰는 습관**을 지금부터 들이는 것이, 이번 프로젝트가
가리키는 가장 확실한 학습 방향이다.
